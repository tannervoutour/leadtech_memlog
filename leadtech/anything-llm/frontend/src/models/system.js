...
const System = {
  ...
  fetchCustomFooterIcons: async function (forceRefresh = false) {
    // Clear the cache if we're forcing a refresh
    if (forceRefresh) {
      window.localStorage.removeItem(this.cacheKeys.footerIcons);
    }
    ...
    if (!forceRefresh && !!data && Date.now() - lastFetched < 3_600_000)
      return { footerData: data, error: null };

    // Add a cache-busting query parameter
    const timestamp = Date.now();
    const { footerData, error } = await fetch(
      `${API_BASE}/system/footer-data?t=${timestamp}`,
      {
        method: "GET",
        cache: "no-cache",
        headers: baseHeaders(),
    ...
    console.log("Received new footer data:", newData); // Debug log
